...
class SendEnvLabelCsvMessagesCommand extends Command
{
    ...
    {
        $this
            ->setName('app:send-label-csv-messages')
            ->setHelp("Sends one message per each CSV chopped file to RabbitMQ.")
            ->setDescription('Loops through the CSV file directory sending one message per each one of '.
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $messages = $this->sendEnvLabelCsvMessagesService->scanDirAndSendMessages();
